...
// Validate password's length is within acceptable range
var isLengthWithinRange = function(pw){
   if ((pw.length >= PW_LENGTH_MIN) && (pw.length <= PW_LENGTH_MAX)){
      console.log(pw.length.toString() + " has correct length within range");
      return true;
   }
 }
...
// //    console.log(generatedPassword);
   ...
   var generatedPassword = ">6w;.+t8-=p3";

   wishlistPassword = generatedPassword; // may not needed
   pwResult.textContent = EMPTY_STRING;

   // Validate the generated password against specs to ensure proper desired length:

   if (isLengthWithinRange(generatedPassword)){
      pwResult.textContent = "From set " + validChars + " the generated pw " + generatedPassword + " has " + generatedPassword.length.toString() + " character - an acceptable length per specs" + DOUBLE_LINE_BREAK; 
    } else {
      pwResult.textContent = "From set " + validChars + " the generated pw " + generatedPassword + " is too short or too long" + DOUBLE_LINE_BREAK; 
    }
   ...
   if (wishlistHasLowercase) {
      ...
      if (found)
      {
         pwResult.textContent += "From set " + validChars + " the generated pw " + generatedPassword + " has at least one lowercase" + DOUBLE_LINE_BREAK;
      } else {
         pwResult.textContent += "From set " + validChars + " the generated pw " + generatedPassword + " has no lowercase" + DOUBLE_LINE_BREAK;
      }
   }

   if (wishlistHasNumber) {
      if (hasNumericCharacter(generatedPassword)){
         pwResult.textContent += "From set " + validChars + " the generated pw " + generatedPassword + " has at least one numeric letter" + DOUBLE_LINE_BREAK;
      } else {
         pwResult.textContent += "From set " + validChars + " the generated pw " + generatedPassword + " has no numeric letter" + DOUBLE_LINE_BREAK;
      }
   }
   ...
   if(wishlistHasSpecialChar){
      ...
      if(detecteCharSpecial(generatedPassword)){
         pwResult.textContent += generatedPassword + " has at least one special character"+ DOUBLE_LINE_BREAK;  
      } else {
         pwResult.textContent += generatedPassword + " has no special characters"+ DOUBLE_LINE_BREAK;  
      }
   }
